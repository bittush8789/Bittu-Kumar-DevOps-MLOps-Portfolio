...
const SkillItem = memo(({ skill }: { skill: { name: string; icon: string } }) => {
  ...
  return (
    <div
      className="flex items-center gap-6 group/item cursor-default transition-none"
    >
      <div className="w-16 h-16 rounded-[24px] glass-panel flex items-center justify-center border-white/5 group-hover/item:border-primary/40 group-hover/item:bg-primary/5 transition-none shadow-xl relative overflow-hidden ring-1 ring-white/5">
        <div className="relative z-10">
          <Icon size={26} className="text-neutral-500 group-hover/item:text-primary transition-none" />
        </div>
        <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-transparent to-transparent opacity-0 group-hover/item:opacity-100 transition-none" />
      </div>
      <div className="flex flex-col">
        <p className="text-lg font-black tracking-tight text-neutral-400 group-hover/item:text-foreground transition-none">{skill.name}</p>
        <div className="w-0 h-[2px] bg-primary/40 group-hover/item:w-full transition-none mt-2 rounded-full" />
      </div>
    </div>
  );
});
...
export function Skills() {
  return (
    <section id="skills" className="section-liquid relative overflow-hidden transition-none">
        ...
        >
          {SKILLS.map((category) => (
            <div key={category.category} className="group">
              <h4 className="text-[13px] font-black tracking-[0.4em] uppercase text-neutral-400 mb-12 pb-6 border-b border-white/5 flex items-center justify-between group-hover:text-primary transition-none">
                {category.category}
                <div 

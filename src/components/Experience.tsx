...
export function Experience() {
  return (
    <section id="experience" className="py-20 md:py-32 relative overflow-hidden transition-none">
...
function ExperienceCard({ exp, idx }: { exp: any, idx: number }) {
  ...
  return (
    ...
    >
      <div className="glass-panel h-full p-8 md:p-12 rounded-[40px] md:rounded-[56px] border-white/5 hover:border-primary/20 transition-none relative overflow-hidden flex flex-col shadow-2xl">
        {/* Background watermark */}
        <div className="absolute -top-10 -right-10 opacity-[0.02] group-hover:opacity-[0.08] transition-none">
          <Icon size={240} />
        </div>

        {/* Header */}
        <div className="flex items-start justify-between gap-6 mb-12 relative z-10">
          <div className="w-20 h-20 rounded-[24px] bg-white/5 border border-white/5 flex items-center justify-center group-hover:bg-primary group-hover:border-primary/30 transition-none shadow-xl shrink-0">
        ...
        {/* Role & Company */}
        <h4 className="text-3xl md:text-4xl font-black mb-3 leading-tight group-hover:text-primary transition-none relative z-10">
          {exp.role}
        </h4>
        ...
        <ul className="space-y-6 mb-12 relative z-10">
          {displayedAchievements.map((achievement: string, i: number) => (
            ...
            >
              ...
              <p className="text-base text-neutral-500 font-medium leading-relaxed group-hover/item:text-foreground transition-none">
                {achievement}
              </p>
            </li>
          ))}
        ...
        {exp.achievements.length > 3 && (
          <button 
            ...
            className="mt-auto text-[11px] font-black tracking-[0.3em] uppercase text-primary hover:text-foreground transition-none flex items-center gap-4 relative z-10 w-fit"
          >
            {showAll ? 'Show Less' : `View All ${exp.achievements.length} Achievements`}
            <div className="w-8 h-[2px] bg-primary/20 group-hover:bg-primary transition-none" />
          </button>
        )}
      </div>
    </div>
